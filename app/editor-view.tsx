...
export function EditorView() {
  ...
  const uploadBlob = async (blob: Blob, filename: string, patch: Record<string, string | number> = {}) => {
    ...
    try { const response = await fetch("/api/assets", { method: "POST", body: form }); const data = await response.json() as any; if (!response.ok) throw new Error(data.error || "Upload failed"); updateSelected({ [imageSlot]: data.asset.url, ...patch }); toast.success("Image saved. Use a public HTTPS image URL before email delivery.", { id: "image-upload" }); }
  ...
  const properties = <PropertyPanel selected={selected} update={updateSelected} move={move} duplicate={duplicate} remove={remove} replace={() => uploadInput.current?.click()} editImage={() => setImageStudioOpen(true)} imageSlot={imageSlot} setImageSlot={setImageSlot} />;
...
function BlockInspector({ block, update, replace, editImage, imageSlot, setImageSlot }: { block: EmailBlock; update: (patch: Record<string, string | number>) => void; replace: () => void; editImage: () => void; imageSlot:string; setImageSlot:(key:string)=>void }) {
  const d = block.data;
  if(block.type==="story"||block.type==="gallery")return <>
    ...
    {block.type==="gallery"&&<Field label="Photo to edit"><div className="flex gap-2">{["imageUrl","image2","image3"].map((key,i)=><Button key={key} variant={imageSlot===key?"default":"outline"} onClick={()=>setImageSlot(key)}>Photo {i+1}</Button>)}</div></Field>}
