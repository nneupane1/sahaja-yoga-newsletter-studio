...
type Props={accountControls?:ReactNode; workspace:any; update:(patch:any)=>Promise<void>; open:(id:string)=>void; events:()=>void};
export function WorkspaceControls({workspace,update,open,events,accountControls}:Props){
 ...
 {panel==="profile"&&<><p className="text-sm text-slate-500">{workspace?.user.email||workspace?.user.identitySource}</p>{accountControls?<><p className="font-semibold">{workspace?.user.displayName}</p>{accountControls}</>:<><label>Full display name<Input value={name} onChange={e=>setName(e.target.value)} autoComplete="name"/></label><Button disabled={busy||!name.trim()} onClick={()=>save({displayName:name})}>Save profile</Button></>}</>}
