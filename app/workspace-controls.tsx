...
export function WorkspaceControls({workspace,update,open,events}:Props){
 ...
 <button className="flex size-10 items-center justify-center rounded-full bg-blue-50 font-bold text-blue-700" aria-label="Your profile" onClick={()=>{setName(workspace?.user.displayName||"");setError("");setPanel("profile");}}>{workspace?.user.initials||"…"}</button>
